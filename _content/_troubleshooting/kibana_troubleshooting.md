...
description: Step-by-step instructions on how to troubleshoot Dashboards/Kibana issues with the Search Guard plugin.
---

<!--- Copyright 2020 floragunn GmbH -->

# Dashboards/Kibana troubleshooting

## Dashboards/Kibana optimization is very slow, stalls or exit unexpectedly

The Dashboards/Kibana [optimization process is shaky](https://github.com/elastic/kibana/issues/19678){:target="_blank"} and problems are typically not related to Search Guard.
...
Dashboards/Kibana fails to start and display 
...
If you use self signed certificate you may see the following error in the Dashboards/Kibana log file:

```
HEAD https://example.com:9200/ => unable to get local issuer certificate
```

This means that Dashboards/Kibana does not trust the self-signed root CA certificate. You can either disable the certificate cerification or install the root CA in Dashboards/Kibana.
...
Either access Dashboards/Kibana with HTTPS instead of HTTP, or set:
...
A common source of error is that a HTTP header required by the configured authentication module(s) is not whitelisted, and thus authentication fails. Since the header is just discarded by Dashboards/Kibana, you won't see any error message.
...
If you want to transmit the JSON web token as request parameter to Dashboards/Kibana, Search Guard needs to  grab the token from the request, and add it to all calls to OpenSearch/Elasticsearch as HTTP request parameter. The reason is that Dashboards/Kibana does not allow to add arbitrary request parameters to HTTP call.
