---
title: Get all trust stores
html_title: Get all trust stores
permalink: elasticsearch-alerting-rest-api-trust-store-get-all
category: signals-rest
order: 852
layout: docs
edition: community
description: Use to retrieve information about trust store.
---

<!--- Copyright 2023 floragunn GmbH -->

# Get all trust stores
{: .no_toc}

{% include toc.md %}


```
GET /_signals/truststores/
```

Load information about all trust stores 


## Responses

### 200 OK

List which contains all defined trust stores in the system.

### 403 Forbidden

The user does not have the required permissions to access the endpoint.

## Permissions

For being able to access the endpoint, the user needs to have the privilege `cluster:admin:searchguard:signals:truststores/findall`.

## Examples

### Get all trust stores

```
GET /_signals/truststores/
```

**Response**

```
200 OK
```

```json
{
  "status": 200,
  "data": [
    {
      "id": "ca-trust-anchor",
      "name": "my trust anchor",
      "raw_pem": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
      "certificates": [
        {
          "pem": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----",
          "subject": "CN=root.ca.number-0.com,OU=SearchGuard,O=index-0",
          "issuer": "CN=root.ca.number-0.com,OU=SearchGuard,O=index-0",
          "not_before": "2023-05-18T15:43:30.000Z",
          "not_after": "2023-06-17T15:43:30.000Z",
          "serial_number": "1"
        }
      ]
    }
  ]
}
```

