...
**tls.truststore_id** The `id` of the [trust store uploaded previously](elasticsearch-alerting-trust-stores) by the REST API. All certificates from the trust store are considered trusted. The parameter is optional and mutually exclusive with `tls.trusted_certs`.

**tls.client_session_timeout** Define TLS session timeout, please see [TLS session timeout](elasticsearch-alerting-trust-stores#tls-session-timeout).
