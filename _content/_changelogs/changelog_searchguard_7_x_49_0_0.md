...
The auth token service is available in the Search Guard Kibana plugin and via a dedicated [REST API](https://test-docs.search-guard.com/latest/search-guard-auth-tokens#creating-auth-tokens).

The auth token service is disabled by default. To use it, you need to [enable and configure](https://test-docs.search-guard.com/latest/search-guard-auth-tokens#configuring-the-search-guard-auth-token-service) it inside `sg_config.yml`.
...
More details:
* [Documentation](https://test-docs.search-guard.com/latest/search-guard-auth-tokens)
...
More information:
* [Documentation for backend configuration](https://test-docs.search-guard.com/latest/openid-json-web-keys)
* [Documentation for frontent configuration](https://test-docs.search-guard.com/latest/kibana-authentication-openid)
...
* [Docs for JWT](https://test-docs.search-guard.com/latest/json-web-tokens#using-only-certain-sections-of-a-jwt-subject-claim-as-user-name)
* [Docs for OIDC](https://test-docs.search-guard.com/latest/openid-json-web-keys#using-only-certain-sections-of-a-jwt-subject-claim-as-user-name)
* [Docs for SAML](https://test-docs.search-guard.com/latest/saml-authentication#using-only-certain-sections-of-a-saml-user-name)
...
The feature for blocking IPs did not work correctly when ES is only reachable by a proxy. While Search Guard can use IPs obtained from a `X-Forward-For` header for IP-based authentication (if `xff` is configured in `sg_config.yml`; see the [documentation](https://test-docs.search-guard.com/latest/authentication-authorization#http) for details), IP blocking did not take such IPs into account.
